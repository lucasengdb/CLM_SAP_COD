 ...
 */
public class UDO4_FieldV_ExternalId extends CLMDummyScriptDefinition {
    ...
    private void gap024_validaCPF() throws ApplicationException {
        
        String cpf = doc.getExternalId();
        cpf = cpf.replace(".", "").replace("-", "");

        if (isCPF(cpf)) {
            doc.setExternalId(imprimeCPF(cpf));
        } else {
            throw getLocalizedApplicationException(session, "mensagem.gap024.erro.cpf_invalido", null);
        }
    }
