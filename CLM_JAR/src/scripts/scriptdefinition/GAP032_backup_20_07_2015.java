...
public class GAP032_backup_20_07_2015 extends CLMDummyScriptDefinition{
	...
	private void logDebug(String mensagem) {
		Logger.debug(Logger.createLogMessage(session).setMethod(
				LOGGER_ID + mensagem));
	...
	private Boolean isUserHasRole(IapiAccountIfc user, String securityProfile)
		...
		this.logInfo("Resultado do que veio do banco tem mais de um "+session.getDbHandle().getResultSet().next());
		this.logInfo("É diferente de TRUE?  "+!session.getDbHandle().getResultSet().next());
		if (!session.getDbHandle().getResultSet().next()) {
			this.logInfo(" Ta passando aqui no session.getDbHandle().getResultSet().next() ");
			ret = true;
		}
	...
	private void gap032_validaAprovadores(String tipoDoc)
		...
		for (int i = 0; i < colabs.size(); i++) {
			...
			if (colab.getCollaboratorRole().equals(
				...
				//INICIO - Acordo comercial 
				if (tipoDoc.equalsIgnoreCase(ACORDO_COMERCIAL)) {
					this.logInfo("ENTROU NO ACORDO COMERCIAL");
					if (representa.getDisplayName().equals(
							REPRESENTS_ADVG_JURIDICO)) {
						if (isUserHasRole(user, ROLE_ADVG_JURIDICO)) {
					...
					if (representa.getDisplayName().equals(
							REPRESENTS_MGR_FINAN_CSA)) {
						this.logInfo("ENTROU NO REPRESENTA FINAN CS - COMERCIAL ACORDO E NÃO SUB");
						
						if (isUserHasRole(user, ROLE_GERENTE_FINANC)) {
							this.logInfo("Não existe a ROLE GERENTE");
							users.add(user.getFirstName());
						}
					}
	...
	public  void inicio() throws ChainedException {
		...
		if (!session.getAccount().getUserName().equalsIgnoreCase("enterprise")
			...
			try {
				...
				this.logError("Ocorreu um erro. Mensagem do erro  " + e.getMessage());
