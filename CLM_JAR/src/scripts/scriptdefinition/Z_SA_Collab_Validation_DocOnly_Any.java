...
public class Z_SA_Collab_Validation_DocOnly_Any extends CLMDummyScriptDefinition {
	...
	public ApplicationException getAppException(String resourceId, Object[] modifiers) {
	...
	private void removeRevisorSemRepresenta() throws ApplicationException, DatabaseException {
		...
		CollaboratorRoleIBeanHomeIfc roleHome = (CollaboratorRoleIBeanHomeIfc) IBeanHomeLocator.lookup(session,
				CollaboratorRoleIBeanHomeIfc.sHOME_NAME);
				...
				if (colab.getCollaboratorType().get() == CollaboratorTypeEnumType.user) {
					...
					if (!user.getUserName().equalsIgnoreCase("WORKFLOWUSER")) {
						if (!(colab.getSource().get() == CollaboratorSourceEnumType.WORKFLOW)) {
							throw getLocalizedApplicationException("mensagem.gap007_e_008.erro_representa_a_definir",
									null);
						}
					}
				} else {
					throw getLocalizedApplicationException("mensagem.gap007_e_008.erro_representa_a_definir", null);
				}
			}
		}
	}
	...
	public void inicio() throws ApplicationException {
		...
		this.logDebug("DEBUG FUNCIONANDO =D");
		...
		if (!session.getAccount().getUserName().equalsIgnoreCase("enterprise")
			...
			try {
				...
				this.logError("Erro mensagem: " + e.getMessage());
	...
	private void gap032_validaAprovadores(String tipoDoc) throws ChainedException, NoConnectionException, SQLException {

		this.logDebug("Log de Debug");
		this.logInfo("INFO");

		String nomes = "";
		IapiAccountIfc user = null;
		List users = new ArrayList();
		ValueListValueIBeanHomeIfc representanteHome = (ValueListValueIBeanHomeIfc) IBeanHomeLocator.lookup(session,
		...
		for (int i = 0; i < colabs.size(); i++) {
			...
			if (colab.getCollaboratorRole().equals(roleAprovadorIfc.getObjectReference())
				...
				} else if (representa.getDisplayName().equals(REPRESENTS_MGR_FINAN_CSA)) {
		...
		if (!users.isEmpty()) {
			...
			throw getAppException("mensagem.gap032.erro_funcao_colaborador_invalida", new Object[] { nomes });
	...
	private Boolean isUserHasRole(IapiAccountIfc user, String securityProfile)
		...
		String sql = "SELECT T3.INTERNAL_NAME FROM " + schema + ".FCI_UPP_USER_ACCOUNT T1, " + schema
				+ ".FCI_UPP_ROLE_REF T2, " + schema + ".FCI_UPP_ROLE T3 " + "WHERE "
				+ "T1.OBJECTID = T2.PARENT_OBJECT_ID AND " + "T2.ROLE_OBJECT_ID = T3.OBJECTID AND " + "T1.NAME = '"
				+ user.getUserName() + "' AND " + "T3.INTERNAL_NAME = '" + securityProfile + "'";
